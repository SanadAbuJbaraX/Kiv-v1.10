...
from kivParseResult import ParseResult
from kivTokens import *
from kivNodes import *
class Parser:
	...
	def parse(self):
		...
		if not res.error and self.current_tok.type != TT_EOF:
			return res.failure(InvalidSyntaxError(
				...
				f"Error at Token '{self.current_tok.value}'"
									))
		return res
	...
	def statement(self):
		...
		if res.error: return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end,
			"Expected int,return,continue,break float, identifier, '+', '-', '()','[]','function','if','for','while'"
		))
	...
	def import_expr(self):
		...
		if self.current_tok.type != TT_STRING:
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				f"Expected string to import after 'import' but got '" + self.current_tok.type + "\'"
			))
		file = self.current_tok
		res.register_advancement()
		self.advance()
		if res.error: return res
		return ImportNode(self.current_tok.pos_start,self.current_tok.pos_end,file)
	def while_expr(self):
		res = ParseResult()
	...
	def atom(self):
		...
		return res.failure(InvalidSyntaxError(
			...
			"Expected int, float, identifier, '+', '-', '()','[]','function','if','for','while'"
		))
	...
	def comp_expr(self):
		...
		node = res.register(self.bin_op(self.arith_expr, (TT_EQEQ, TT_NE, TT_LT, TT_GT, TT_LTE, TT_GTE)))
	...
	def expr(self):
		...
		if self.current_tok.matches(TT_KEYWORD, 'var'):
			...
			expr = res.register(self.expr())
			if res.error: return res
			if var_name.value not in KEYWORDS:
				return res.success(VarAssignNode(var_name, expr))
			return res.failure(InvalidSyntaxError(
					var_name.pos_start, var_name.pos_end,
					f"Invalid Identifier '{var_name.value}'\n'{var_name.value}' is pre-defined (keywords,builtins...etc)"
				))
		...
		if res.error:
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				"Expected 'var', int, float, identifier, '+', '-', '()','[]' or 'not','function','if','for','while'"
			))
